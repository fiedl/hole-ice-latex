...
Using modified angular-acceptance curves for the optical modules to
...
This approach does not rely on specific assumptions about the properties
...
The \noun{IceCube Simulation Framework} includes two independent
photon-propagation-simulation tools, \clsim and
\ppc (\noun{Photon Propagation Code}).
...
Since both propagation softwares, \ppc and \clsim with the new
...
At a rough view, both simulations produce similar effective
...
In the \noun{DARD} study (\noun{Data Acquisition for a flasheR DOM}),
...
The \noun{DARD} study assumes a fixed hole-ice radius of \(r=30\cm\).
...
Using the new direct photon propagation though hole ice with \clsim, one
can simulate the propagation through hole ice with parameters suggested
by \noun{DARD}, scan the angular acceptance of an optical module within
the simulation, and compare the result to the existing a priori
...
The \noun{SpiceHD} study
(\noun{South Pole ICE model with Hole ice fitting and Direct detection})
...
Comparing the flasher data to simulations with different hole-ice
...
Using the new direct photon propagation though hole ice with \clsim, one
can simulate the propagation through hole ice with parameters suggested
by \noun{SpiceHD}, scan the effective angular acceptance of an optical
...
\noun{SpiceHD} uses \ppc to propagate the photons.
\cite{martinspicehddard}
...
\noun{SpiceHD}'s contour plot (figure \ref{fig:ahCoHee4} a) smoother and
...
\ref{sec:flasher} to match the scale of \noun{SpiceHD}, as shown in
...
The \noun{SpiceHD} contours are compatible to camera observations
...
The \noun{SpiceHD} flasher study is compatible to camera observations.
...
calibration study \noun{DARD}, angular-acceptance curves from direct
...
This leaves no obvious conclusion on which current hole-ice model is to
