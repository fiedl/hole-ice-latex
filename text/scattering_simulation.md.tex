...
The photon-propagation simulation records the starting point, each scattering point, and the final trajectory point of each photon. The photon trajectories can be visualized using the \steamshovel event display software.
...
Figure \ref{fig:Uo8kuo2z} shows the \steamshovel visualizations of the simulation described above for different hole-ice scattering lengths.
...
The mean scattering angle in the bulk ice as well as in the hole ice is assumed as \ang{20} \cite{escawiki}. Therefore, for a weak hole ice with a large scattering length, the photons are deflected within the hole ice. See figure \ref{fig:Uo8kuo2z} (c) in comparison to (b).
...
% This image is taken from the talk slides.
\begin{figure}[htbp]
