...
\begin{figure}[htbp]
  ...
  \subcaptionbox{Angular acceptance $a(\eta)$: Sensitivity of the DOM to incoming photons depending on the polar angle $\eta$, relative to the sensitivity to photons from the optimal incoming angle $\eta = 0, \cos \eta = 1$. The ``nominal'' curve $a\dom(\eta)$ does not consider hole-ice effects and is based on lab measurements. The ``hole ice'' curve $a\domhi(\eta)$ effectively contains hole-ice effects and is based on simulations. Plot taken from \cite{icepaper}, figure 7.}{\halfimage{ice-paper-fig7}}
