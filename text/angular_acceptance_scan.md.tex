...
Due to the limitation of computational resources, rather than simulating plane waves with infinite extend, an arbitrary extent $e$ of the plane is chosen. \authorname{Rongen} \cite{martindardupdate} compares the influence of the plane extent $e$ on the angular-acceptance curves (\cite{martindardupdate}, slides 6 and 9). The angular-acceptance simulations described in the section arbitrarily choose a plane extent $e = 1\m$ and a start distance $d = 1\m$.

\todo{\cite{rongenswedishcamera}: angular-acceptance curve stable for beams larger than 1m radius, placed more than 2.5m away from dom}
...
\todo{Talk about the bulk-ice systematics: \issue{108}}
